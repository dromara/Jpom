<template>
    <div>
        ...
        <a-modal v-model="editBuildVisible" title="编辑构建" @ok="handleEditBuildOk" width="40%" :maskClosable="false">
            <a-form-model ref="editBuildForm" :rules="rules" :model="temp" :label-col="{ span: 3 }"
                    ...
                    <a-row>
                        <a-col :span="18">
                            <a-select v-model="temp.branchName" placeholder="请先填写仓库地址和账号信息">
                                <a-select-option v-for="branch in branchList" :key="branch">{{
                                    branch
                                    }}
                                </a-select-option>
                            </a-select>
                ...
                <a-form-model-item label="构建命令" prop="script">
                    <a-input v-model="temp.script" type="textarea" :auto-size="{ minRows: 2, maxRows: 6 }" allow-clear
                             placeholder="构建执行的命令(非阻塞命令)，如：mvn clean package"/>
                </a-form-model-item>
                ...
                <a-form-model-item v-if="temp.releaseMethod === 2" label="发布项目" prop="releaseMethodDataId">
                    <a-cascader v-model="temp.releaseMethodDataIdList" :options="cascaderList"
                ...
                <a-form-model-item v-if="temp.releaseMethod === 3" label="发布命令" prop="releaseCommand">
                    <a-input v-model="temp.releaseCommand" allow-clear :auto-size="{ minRows: 2, maxRows: 10 }"
                             type="textarea" :rows="3" placeholder="发布执行的命令(非阻塞命令),一般是启动项目命令 如：ps -aux | grep java"/>
                </a-form-model-item>
                <a-form-model-item v-if="temp.releaseMethod === 2 || temp.releaseMethod === 3" label="清空发布"
                                   prop="clearOld">
        ...
        <a-modal v-model="triggerVisible" title="触发器" :footer="null" :maskClosable="false">
            <a-form-model ref="editTriggerForm" :rules="rules" :model="temp" :label-col="{ span: 6 }"
                          :wrapper-col="{ span: 16 }">
                <a-form-model-item label="触发器地址" prop="triggerBuildUrl">
                    <a-input v-model="temp.triggerBuildUrl" type="textarea" readOnly :rows="3" style="resize: none;"
                             placeholder="触发器地址"/>
                </a-form-model-item>
                <a-row>
                    <a-col :span="6"></a-col>
