<template>
	<div>
		...
		<a-table :data-source="list" :loading="loading" :columns="columns" :pagination="false" bordered
			...
			<template slot="operation" slot-scope="text, record">
				...
				<a-dropdown>
					...
					<a-menu slot="overlay">
						<a-menu-item>
							<a-button type="primary" @click="handleMonitor(record)" v-show="record.runMode!=='File'"
												:disabled="!record.status">监控
							</a-button>
						</a-menu-item>
						<a-menu-item>
							<a-button type="primary" @click="handleReplica(record)" v-show="record.runMode!=='File'" :disabled="!record.javaCopyItemList">副本集
							</a-button>
						</a-menu-item>
						<a-menu-item>
		...
		<a-modal v-model="editProjectVisible" width="800px" title="编辑项目" @ok="handleEditProjectOk" :maskClosable="false">
			<a-form-model ref="editProjectForm" :rules="rules" :model="temp" :label-col="{ span: 6 }"
										:wrapper-col="{ span: 14 }">
				...
				<a-form-model-item label="JVM 参数" prop="jvm" v-show="temp.runMode !== 'File'">
					<a-textarea v-model="temp.jvm" :auto-size="{ minRows: 3, maxRows: 3 }"
											placeholder="jvm参数,非必填.如：-Xms512m -Xmx512m"/>
				</a-form-model-item>
				<a-form-model-item label="args 参数" prop="args" v-show="temp.runMode !== 'File'">
					<a-textarea v-model="temp.args" :auto-size="{ minRows: 3, maxRows: 3 }"
											placeholder="Main 函数 args 参数，非必填. 如：--service.port=8080"/>
				</a-form-model-item>
				<!-- 副本信息 -->
				<a-row v-for="replica in temp.javaCopyItemList" :key="replica.id">
					<a-form-model-item :label="`副本 ${replica.id} JVM 参数`" prop="jvm">
						<a-textarea v-model="replica.jvm" :auto-size="{ minRows: 3, maxRows: 3 }" class="replica-area"
												placeholder="jvm参数,非必填.如：-Xms512m -Xmx512m"/>
					</a-form-model-item>
					<a-form-model-item :label="`副本 ${replica.id} args 参数`" prop="args">
						<a-textarea v-model="replica.args" :auto-size="{ minRows: 3, maxRows: 3 }" class="replica-area"
												placeholder="Main 函数 args 参数，非必填. 如：--service.port=8080"/>
					</a-form-model-item>
		...
		<a-drawer :title="drawerTitle" placement="right" width="85vw"
			...
			<file v-if="drawerFileVisible" :nodeId="node.id" :projectId="temp.id" :runMode="temp.runMode"
						:absPath="(temp.whitelistDirectory || '') + (temp.lib || '')" @goConsole="goConsole"/>
		</a-drawer>
		<!-- 项目控制台组件 -->
		<a-drawer :title="drawerTitle" placement="right" width="85vw"
