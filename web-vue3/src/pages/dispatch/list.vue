        ...
        <a-collapse v-show="noFileModes.includes(temp.runMode)">
          <a-collapse-panel v-for="nodeId in temp.nodeIdList" :key="nodeId" :header="nodeNameMap[nodeId] || nodeId">
            ...
            <a-form-item name="token" v-show="noFileModes.includes(temp.runMode)">
              ...
              <a-input
                v-model="temp[`${nodeId}_token`]"
                placeholder="项目启动,停止,重启都将请求对应的地址,非必填，GET请求"
              />
            </a-form-item>
                        ...
                        <a-col :span="4">
                          <a-tooltip placement="topLeft" title="已经添加成功的副本需要在副本管理页面去删除">
                            <a-button
                              ...
                              type="danger"
                              @click.stop="handleDeleteReplica(nodeId, replica)"
                              >删除</a-button
                            >
                          </a-tooltip>
                        </a-col>
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          width: '90px',
          ellipsis: true,
          scopedSlots: { customRender: 'outGivingProject' }
        },
        {
          title: '分发后',
          dataIndex: 'afterOpt',
          ellipsis: true,
          width: '150px',
          scopedSlots: { customRender: 'afterOpt' }
        },
        {
          title: '清空发布',
          dataIndex: 'clearOld',
          ...
          ellipsis: true,
          width: '100px',
          scopedSlots: { customRender: 'clearOld' }
        },
        {
          title: '间隔时间',
