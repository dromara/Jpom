  ...
  <div>
    ...
    <a-layout class="file-layout node-full-content">
      ...
      <a-layout-content class="file-content">
                ...
                <template slot="title">
                  ...
                  <div>
                    <ul>
                      ...
                      <li>
                        linux 服务器如果为编译安装则需要将 nginx 服务名称配置到 nginx执行文件的绝对路径，如
                        <b>/usr/local/nginx/sbin/nginx</b>
                      </li>
                      <li>
                        windows 服务器是需要提前安装 nginx 并配置服务,默认执行 net start xxxx、net stop xxxx、net、sc
                        query xxxx
                      </li>
                    </ul>
                  </div>
          ...
          <template slot="operation" slot-scope="text, record">
            ...
            <a-popover title="删除确认" v-if="record.name.endsWith('.conf')">
              <template slot="content">
                ...
                <p>
                  <a-button size="small" type="danger" @click="handleDelete(record, 'real', 'none')">永久删除</a-button>
                </p>
              </template>
            ...
            <a-popover title="还原" v-else>
              <template slot="content">
                ...
                <p>
                  <a-button size="small" type="danger" @click="handleDelete(record, 'real', 'back')">永久删除</a-button>
                </p>
              </template>
        ...
        <a-modal
          destroyOnClose
          v-model="editNginxVisible"
          title="编辑 Nginx 配置文件"
          @ok="handleEditNginxOk"
          :maskClosable="false"
          width="70vw"
        >
          <a-form-model
          ...
          >
            ...
            <a-form-item label="配置内容" prop="context">
              <code-editor
        ...
        <a-modal
          destroyOnClose
          v-model="editNginxNameVisible"
          title="编辑 Nginx 服务名称"
          @ok="handleEditNginxNameOk"
          :maskClosable="false"
          width="500px"
        >
          <a-form-model
