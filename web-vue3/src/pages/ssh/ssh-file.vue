        ...
        <template slot="title">
          <a-space>
            ...
            <a-button size="small" :disabled="!this.tempNode.nextPath" type="primary" @click="loadFileList()"
              >刷新</a-button
            >
            <a-button size="small" :disabled="!this.tempNode.nextPath" type="danger" @click="handleDeletePath()"
              >删除</a-button
            >
            <div>
              文件夹：
            ...
            <span v-if="nowPath">当前目录:{{ this.nowPath }}</span>
            <!-- <span v-if="nowPath">{{ this.tempNode.parentDir }}</span> -->
          </a-space>
        </template>
        <a-tooltip
          slot="name"
