<template>
  <div>
    <a-page-header :backIcon="false">
      ...
      <template slot="extra">
        <a-button @click="init" icon="reload"> </a-button>
      </template>
    </a-page-header>
    <a-divider dashed />
    <a-row>
    ...
    <a-row :gutter="[16, 16]">
      <a-col :span="6">
        <a-card size="small">
          <template #title>
            数据统计 <a-tooltip title="当前工作空间关联数据统计"><a-icon type="question-circle" /></a-tooltip>
          </template>
          <a-list :data-source="statNames">
            <a-list-item slot="renderItem" slot-scope="item"> {{ item.name }}：{{ statData[item.field] || "-" }} </a-list-item>
          </a-list>
        </a-card>
      </a-col>
      <a-col :span="6">
        <a-card size="small">
          <template #title>
            构建日志 <a-tooltip title="当前工作空间您触发的构建记录"><a-icon type="question-circle" /></a-tooltip>
          </template>
          <a-timeline v-if="buildLog && buildLog.length">
            <a-timeline-item v-for="item in buildLog" :key="item.id" :color="statusColor[item.status]">
              <a-space direction="vertical" :size="1">
                <div>{{ parseTime(item.startTime) }} ~ {{ parseTime(item.endTime) }}</div>
                <a-space>
      ...
      <a-col :span="6">
        <a-card size="small">
          ...
          <a slot="extra" href="#" @click="handleUserlog(2)">more</a>
          <a-timeline>
      ...
      <a-col :span="6">
        <a-card size="small">
          <template #title>
            操作日志 <a-tooltip title="系统中您所有操作日志"><a-icon type="question-circle" /></a-tooltip>
          </template>
          <a slot="extra" href="#" @click="handleUserlog(1)">more</a>
          <a-timeline v-if="operateLog && operateLog.length">
            <a-timeline-item :color="item.optStatus === 200 ? 'green' : 'red'" v-for="(item, index) in operateLog" :key="index">
              <a-space direction="vertical" :size="1">
                <div>{{ parseTime(item.createTimeMillis) }}</div>
                <a-space>
...
export default {
  ...
  computed: {
    ...mapGetters(["getUserInfo", "getWorkspaceId"]),
  },
  data() {
    return {
      triggerBuildTypeMap,
