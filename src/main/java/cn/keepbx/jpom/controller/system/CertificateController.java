...
public class CertificateController extends BaseController {
    ...
    @ResponseBody
    public String addCertificate(String id, String path) throws IOException {
        if (StrUtil.isEmpty(id)) {
            return JsonMessage.getString(400, "id不能为空");
        }
        if (StrUtil.isEmpty(path)) {
            return JsonMessage.getString(400, "路径不能为空");
        }
        ...
        if (!file.exists()) {
            ...
            if (!mkdirs) {
                return JsonMessage.getString(400, "创建文件夹失败");
            }
        }
    ...
    @ResponseBody
    public String updateCertificate(String id, String path) throws IOException {
        if (StrUtil.isEmpty(id)) {
            return JsonMessage.getString(400, "修改证书失败");
        }
        if (StrUtil.isEmpty(path)) {
            return JsonMessage.getString(400, "路径不能为空");
        }
