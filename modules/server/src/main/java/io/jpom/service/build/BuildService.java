...
public class BuildService extends BaseOperService<BuildModel> implements BaseDynamicService {
	 ...
	 * @param userModel 用户
	 * @param id        id
	 * @param delay     延迟执行 （单位秒）
	 * @return json
	 */
	public String start(UserModel userModel, String id, Integer delay) {
		...
		BuildManage.create(item, userModel, delay);
		if (delay == null || delay <= 0) {
			return JsonMessage.getString(200, "开始构建中", item.getBuildId());
		}
		return JsonMessage.getString(200, "延迟" + delay + "秒后开始构建", item.getBuildId());
	}
